<template>
    <div>
        <div class="row m-0 mb-2 pb-2">
            <div class="col-12 px-md-2 px-0 mb-4 tienda position-relative w-100 px-md-5 pt-md-3">
                <nav-expresso class="position-absolute w-calc"></nav-expresso>
                <div class="d-flex justify-content-center align-items-center vh-tienda">
                    <div class="text-center text-white w-md-50-sm-95 pt-sm-3">
                        <div class="display-4 title">
                            Tienda <span>Expresso</span>.
                            <br>Exelencia en platillos.
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-12 h1 font-weight-bold text-center mb-4">
                <div class="form-group">
                    <input type="text" v-model="buscar" class="form-control rounded-pill d-inline w-50">
                </div>
            </div>
            <div class="col-12 px-5">
                <div class="row m-0 p-0">
                    <div class="col-lg-4 col-md-4 col-sm-6 mb-3" v-if="buscador(value.producto)" v-for="value in tienda">
                ...
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer d-flex justify-content-center">
                            <button type="button" class="btn btn-info rounded-pill" data-dismiss="modal" v-on:click="add()">Agregar al Carrito</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-if="enCarrito != ''">
            <ul class="list-group fixed-bottom list-fixed hidexs" id="list_list">
                <li class="list-group-item pb-1 px-1 text-center">
                    <div class="row m-0">
                        ...
                        <div class="col-3 d-flex justify-content-end">
                            ...
                            v-on:click="cerrar_abrir(1)">X</button>
                        </div>
                    </div>
                </li>
                <li class="list-group-item pb-1 px-1">
                    <div class="row m-0">
                        <div class="col-4">
                            Producto
                        </div>
                ...
                <li class="list-group-item py-1 px-1" v-for="carrito in enCarrito">
                    <div class="row m-0">
                        <div class="col-4">
                            {{carrito.producto}}
                        </div>
            ...
            <button type="button" class="btn btn-danger rounded-pill shows fixed-bottom list-fixed" id="button_list"
...
export default {
    mounted() {
        ...
        progreso.forEach(element => {
            let contenedor = element.getAttribute('content');
            let id = element.getAttribute('id');
            styles.innerHTML += `#${id}.circular-progress::before{content:'${contenedor}'}`;
        });
        this.products();
    ...
    data() {
        return {
            contador: 1,
            info: [
                {info: 1,titulo: 'Repartidores'},
                {info: 2,titulo: 'Marketing'},
                {info: 3,titulo: 'Comercios asociados'},
            ],
            tienda: '',
            cantidad: '',
            producto: '',
    ...
    methods: {
        ...
        items(value){
            ...
            if(value == 1){
                ...
                if(this.recibo < 1){
                    alertify.error("No hay productos en el carrito");
                    modal.click();
                }
            }
            else if(value == 3){
        ...
        cerrar_abrir(value){
            ...
            boton = document.getElementById('button_list');
            if(value == 1){
        ...
        buscador(value){
            return value.toLocaleLowerCase().indexOf(this.buscar.toLocaleLowerCase()) >= 0;
        },
        alert(){
            alertify.error("Esta función no esta disponible");
        },
        type(){
